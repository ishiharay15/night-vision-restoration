...
#Import YOLO Model
network = cv2.dnn.readNetFromDarknet('model1/LLFlow/YOLO/yolo_files/yolov3.cfg', 'model1/LLFlow/YOLO/yolo_files/yolov3.weights')
layers = network.getLayerNames()
yolo_layers = ['yolo_82', 'yolo_94', 'yolo_106']

# Path to the YOLO class names file
yolo_classes_file = '/home/tamngu2/model1/LLFlow/YOLO/yolo_files/coco.names'  # Replace with the actual path to your YOLO class names file
...
parser.add_argument('--input', help='Path to folder containing input and output images', required=True)
...
output_folder = os.path.join('./out_imgs/', args.input)
os.makedirs(os.path.join('out_imgs', args.input), exist_ok=True)
